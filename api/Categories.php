...
class Categories extends Simpla
{
	...
	private function init_categories()
	{
		...
		$query = $this->db->placehold("SELECT c.id, c.clear, c.show_products, c.group_options_in_price, c.parent_id, c.name, c.description, c.url, c.meta_title, c.meta_keywords, c.meta_description, c.image, c.visible, c.position
	...
	public function get_related_products($category_id = array())
	{
	...
	public function add_related_product($category_id, $related_id, $position=0)
	{
		$query = $this->db->placehold("INSERT IGNORE INTO __c_related_products SET category_id=?, related_id=?, position=?", $category_id, $related_id, $position);
		$this->db->query($query);
		return $related_id;
	}
	
	// Удаление связанного товара
	public function delete_related_product($category_id, $related_id)
	{
		$query = $this->db->placehold("DELETE FROM __c_related_products WHERE category_id=? AND related_id=? LIMIT 1", intval($category_id), intval($related_id));
		$this->db->query($query);
	}

	public function get_pin_products($category_id = array())
	{
		if(empty($category_id))
			return array();
	...
	// Функция возвращает связанные товары
	public function add_pin_product($category_id, $related_id, $position=0)
	{
		$query = $this->db->placehold("INSERT IGNORE INTO __c_pin_products SET category_id=?, related_id=?, position=?", $category_id, $related_id, $position);
		$this->db->query($query);
		return $related_id;
	}
	
	// Удаление связанного товара
	public function delete_pin_product($category_id, $related_id)
	{
		$query = $this->db->placehold("DELETE FROM __c_pin_products WHERE category_id=? AND related_id=? LIMIT 1", intval($category_id), intval($related_id));
		$this->db->query($query);
	}
}