...
class Products extends Simpla
{
	...
	*/
	...
	{		
		...
		if(isset($filter['dop']))
			$is_dop_filter = $this->db->placehold('AND p.dop=?', intval($filter['dop']));
	...
	*/	
	...
	//Добавлен фильтр по доп. опциям (dop) и вспомогательному оборудованию(featured)
	public function get_related_products($product_id = array(), $filter = array())
	{
		if(empty($product_id))
			return array();

		$product_id_filter = $this->db->placehold('AND rp.product_id in(?@)', (array)$product_id);

		if(isset($filter['dop']))				
			$is_dop_filter = $this->db->placehold('AND p.dop = ?', $filter['dop']);
		
		if(isset($filter['featured']))				
			$is_featured_filter = $this->db->placehold('AND p.featured = ?', $filter['featured']);
		
		$query = $this->db->placehold("SELECT DISTINCT rp.product_id, rp.related_id, rp.position, p.dop, p.featured
					FROM __related_products rp LEFT JOIN __products p ON p.id = rp.related_id
					WHERE 
					1
					$product_id_filter   
					$is_dop_filter   
	...
	//Функция возвращает связанные товары
	//Добавлен фильтр по доп. опциям (dop) и вспомогательному оборудованию(featured)
	public function get_excluded_related_products($product_id = array(), $filter = array())
	{
		if(empty($product_id))
			return array();
		...
		$query = $this->db->placehold("SELECT DISTINCT product_id, related_id, position
					FROM __excluded_related WHERE 1 $product_id_filter
					ORDER BY position       
					");
		$this->db->query($query);
		return $this->db->results();
	...
	// Функция возвращает связанные товары
	public function add_excluded_related_product($product_id, $related_id, $position=0)
	{
		$query = $this->db->placehold("INSERT IGNORE INTO __excluded_related SET product_id=?, related_id=?, position=?", $product_id, $related_id, $position);
		$this->db->query($query);
		$query = $this->db->placehold("INSERT IGNORE INTO __excluded_related SET product_id=?, related_id=?, position=?", $related_id, $product_id, $position);
		$this->db->query($query);
		return $related_id;
	}
	
	// Удаление связанного товара
	public function delete_excluded_related_product($product_id, $related_id)
	{
		$query = $this->db->placehold("DELETE FROM __excluded_related WHERE product_id=? AND related_id=? LIMIT 1", intval($product_id), intval($related_id));
		$this->db->query($query);
		$query = $this->db->placehold("DELETE FROM __excluded_related WHERE product_id=? AND related_id=? LIMIT 1", intval($related_id), intval($product_id));
		$this->db->query($query);
	}
