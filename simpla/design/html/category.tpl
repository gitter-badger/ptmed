...
$(function() {
	...
	$("input#related_products").autocomplete({
		//serviceUrl:'ajax/search_products.php?category_id_filter='+$("input#accessories_category_id").val(),
	...
	// Удаление связанного товара
	$(".pin_products a.delete").live('click', function() {
		 $(this).closest("div.row").fadeOut(200, function() { $(this).remove(); });
		 return false;
	});
	...
	// Добавление связанного товара 
	//serviceUrl:'ajax/search_products.php?category_id_filter='+$("input#category_id").val(),
	var new_pin_product = $('#new_pin_product').clone(true);
	$('#new_pin_product').remove().removeAttr('id');
 
	$("input#pin_products").autocomplete({
		serviceUrl:'ajax/search_products.php',
		minChars:0,
		noCache: false, 
		onSelect:
			function(suggestion){
				$("input#pin_products").val('').focus().blur(); 
				new_item = new_pin_product.clone().appendTo('.pin_products');
				new_item.removeAttr('id');
				new_item.find('a.related_product_name').html(suggestion.data.name);
				new_item.find('a.related_product_name').attr('href', 'index.php?module=ProductAdmin&id='+suggestion.data.id);
				new_item.find('input[name*="pin_products"]').val(suggestion.data.id);
				if(suggestion.data.image)
					new_item.find('img.product_icon').attr("src", suggestion.data.image);
				else
					new_item.find('img.product_icon').remove();
...
<input type=hidden name="accessories_category_id" id="accessories_category_id" value="{$accessories_category_id}">
<input type=hidden name="category_id" id="category_id" value="{$category->id}">
	...
	<div id="name">
		<div class="checkbox">
			...
			<input name=clear value='1' type="checkbox" id="clear_checkbox" {if $category->clear}checked{/if}/> <label for="clear_checkbox">Прозрачная</label>
			<input name=show_products value='1' type="checkbox" id="show_products_checkbox" {if $category->show_products}checked{/if}/> <label for="show_products_checkbox">Выводить товары</label>
			<input name=group_options_in_price value='1' type="checkbox" id="group_options_in_price_checkbox" {if $category->group_options_in_price}checked{/if}/> <label for="group_options_in_price_checkbox">Не группировать вспом. оборудование в прайс-листе</label>
		</div>
	</div> 
	...
	<div id="column_left">
		...
		*}
		<div class="block layer">
			<h2>Прибитые товары</h2>
			<div id=list class="sortable pin_products">
				{foreach from=$pin_products item=pin_product}
				<div class="row">
					<div class="move cell">
						<div class="move_zone"></div>
					</div>
					<div class="image cell">
					<input type=hidden name=pin_products[] value='{$pin_product->id}'>
					<a href="{url id=$pin_product->id}">
					<img class=product_icon src='{$pin_product->images[0]->filename|resize:35:35}'>
					</a>
					</div>
					<div class="name cell">
					<a href="{url id=$pin_product->id}">{$pin_product->name}</a>
					</div>
				...
				{/foreach}
				<div id="new_pin_product" class="row" style='display:none;'>
					<div class="move cell">
						<div class="move_zone"></div>
					</div>
					<div class="image cell">
					<input type=hidden name=pin_products[] value=''>
			...
			<input type=text name=related id='pin_products' class="input_autocomplete" placeholder='Выберите товар чтобы добавить его'>
		</div>
