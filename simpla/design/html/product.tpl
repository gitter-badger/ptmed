...
$(function() {
	var filter;
	//При загрузке страницы проверим, является ли товар доп. опцией или вспомогательным оборудованием
	filter = check_dop_vspom();

	//Товар может быть просто товаром, а также вспомогательным оборудованием и доп. опцией.
	//Одновременно доп. опцией и вспомогательным оборудованием он быть не может.
	//Поэтому надо проконтролировать состояние чекбоксов dop_checkbox и featured_checkbox - они не могут быть одновременно "зажжены"
	$("#dop_checkbox").change(function(){
		if ($("#dop_checkbox").is(":checked"))
			$("#featured_checkbox").prop("checked", false);
		//Доп. опция не может сама иметь доп. опций и вспомогательного оборудования - поэтому проверим и внесем коррективы
		filter = check_dop_vspom();
	});
	$("#featured_checkbox").change(function(){
		if ($("#featured_checkbox").is(":checked"))
			$("#dop_checkbox").prop("checked", false);
		//Вспомогательное оборудование не может сама иметь доп. опций и вспомогательного оборудования - поэтому проверим и внесем коррективы
		filter = check_dop_vspom();
	});
	...
	// Удаление связанного товара - взаимоисключающей опции
	$(".related_products_exclude a.delete").live('click', function() {
		 $(this).closest("div.row").fadeOut(200, function() { $(this).remove(); });
		 return false;
	});
 

	// Добавление связанного товара - взаимоисключающей опции
	var new_related_product_exclude = $('#new_related_product_exclude').clone(true);
	$('#new_related_product_exclude').remove().removeAttr('id');
 
	$("input#related_products_exclude").autocomplete({
		serviceUrl:'ajax/search_products.php?'+filter+'=1&this='+$("this").val(),
		minChars:0,
		noCache: false, 
		onSelect:
			function(suggestion){
				$("input#related_products_exclude").val('').focus().blur(); 
				new_item = new_related_product_exclude.clone().appendTo('.related_products_exclude');
				new_item.removeAttr('id');
				new_item.find('a.related_product_name').html(suggestion.data.name);
				new_item.find('a.related_product_name').attr('href', 'index.php?module=ProductAdmin&id='+suggestion.data.id);
				new_item.find('input[name*="related_products_exclude"]').val(suggestion.data.id);
	...
	// Удаление связанного товара - доп. опции
	$(".related_products_dop a.delete").live('click', function() {
		 $(this).closest("div.row").fadeOut(200, function() { $(this).remove(); });
		 return false;
	});
 

	// Добавление связанного товара - доп. опции
	var new_related_product_dop = $('#new_related_product_dop').clone(true);
	$('#new_related_product_dop').remove().removeAttr('id');
 
	$("input#related_products_dop").autocomplete({
		serviceUrl:'ajax/search_products.php?is_dop_filter=1',
		minChars:0,
		noCache: false, 
		onSelect:
			function(suggestion){
				$("input#related_products_dop").val('').focus().blur(); 
				new_item = new_related_product_dop.clone().appendTo('.related_products_dop');
				new_item.removeAttr('id');
				new_item.find('a.related_product_name').html(suggestion.data.name);
				new_item.find('a.related_product_name').attr('href', 'index.php?module=ProductAdmin&id='+suggestion.data.id);
				new_item.find('input[name*="related_products_dop"]').val(suggestion.data.id);
				if(suggestion.data.image)
					new_item.find('img.product_icon').attr("src", suggestion.data.image);
				else
					new_item.find('img.product_icon').remove();
	...
	// Удаление связанного товара - вспомогательного оборудования
	$(".related_products_featured a.delete").live('click', function() {
		 $(this).closest("div.row").fadeOut(200, function() { $(this).remove(); });
		 return false;
	});
 

	// Добавление связанного товара - вспомогательного оборудования
	// serviceUrl:'ajax/search_products.php?is_featured_filter=1&product_id_filter='+$("#this").val(),
	var new_related_product_featured = $('#new_related_product_featured').clone(true);
	$('#new_related_product_featured').remove().removeAttr('id');
 
	$("input#related_products_featured").autocomplete({
		serviceUrl:'ajax/search_products.php?is_featured_filter=1',
		minChars:0,
		noCache: false, 
		onSelect:
			function(suggestion){
				$("input#related_products_featured").val('').focus().blur(); 
				new_item = new_related_product_featured.clone().appendTo('.related_products_featured');
				new_item.removeAttr('id');
				new_item.find('a.related_product_name').html(suggestion.data.name);
				new_item.find('a.related_product_name').attr('href', 'index.php?module=ProductAdmin&id='+suggestion.data.id);
				new_item.find('input[name*="related_products_featured"]').val(suggestion.data.id);
				if(suggestion.data.image)
					new_item.find('img.product_icon').attr("src", suggestion.data.image);
				else
					new_item.find('img.product_icon').remove();
...
//Функция проверяет, является ли товар доп. опцией или вспомогательным оборудованием
//и в зависимости от результата модифицирует интерфейс страницы
function check_dop_vspom()
{
	var filter;

	if ( $("#dop_checkbox").is(":checked"))
		filter = 'is_dop_filter';
	else if ( $("#featured_checkbox").is(":checked") )
		filter = 'is_featured_filter';

	$("#exclude_options").hide();
	if ( $("#dop_checkbox").is(":checked") || $("#featured_checkbox").is(":checked") )
	{
		$("#dop_options").hide();
		$("#featured_options").hide();
		$("#exclude_options").show();
	}
	else
	{
		$("#dop_options").show();
		$("#featured_options").show();
		$("#exclude_options").hide();
		//перенесем вспом оборудование в категории
		//$("#featured_options").hide();
	}
...
<input type=hidden name="session_id" value="{$smarty.session.id}">
 	...
 	<div id="name">
		...
		<input id="this" name=id type="hidden" value="{$product->id|escape}"/> 
	</div> 
	<div id="name">
		<div class="checkbox">
		...
		<div class="checkbox">
			<input name=featured value="1" type="checkbox" id="featured_checkbox" {if $product->featured}checked{/if}/> <label for="featured_checkbox">Вспомогательное оборудование</label>
		</div>
		<div class="checkbox">
			<input name=dop value="1" type="checkbox" id="dop_checkbox" {if $product->dop}checked{/if}/> <label for="featured_checkbox">Дополнительная опция</label>
		</div>
	</div> 
	...
	<div id="column_right">
		...
		<div class="block layer" id="dop_options">
			<h2>Дополнительные опции</h2>
			<div id=list class="sortable related_products_dop">
				{foreach from=$related_products item=related_product}
				{* Если связанный товар является доп. опцией, то выводим его*}
				{if $related_product->dop}
				<div class="row">
					<div class="move cell">
						<div class="move_zone"></div>
					</div>
					<div class="image cell">
					<input type=hidden name=related_products_dop[] value='{$related_product->id}'>
				...
				{/if}
				{/foreach}
				<div id="new_related_product_dop" class="row" style='display:none;'>
					<div class="move cell">
						<div class="move_zone"></div>
					</div>
					<div class="image cell">
					<input type=hidden name=related_products_dop[] value='123'>
			...
			<input type=text name=related id='related_products_dop' class="input_autocomplete" placeholder='Выберите товар чтобы добавить его'>
		</div>

		<div class="block layer" id="featured_options" style="display:none;">
			<h2>Вспомогательное оборудование</h2>
			<div id=list class="sortable related_products_featured">
				{foreach from=$related_products item=related_product}
				{* Если связанный товар является вспомогательным оборудованием, то выводим его*}
				{if $related_product->featured}
				<div class="row">
					<div class="move cell">
						<div class="move_zone"></div>
					</div>
					<div class="image cell">
					<input type=hidden name=related_products_featured[] value='{$related_product->id}'>
					<a href="{url id=$related_product->id}">
					<img class=product_icon src='{$related_product->images[0]->filename|resize:35:35}'>
					</a>
					</div>
				...
				{/if}
				{/foreach}
				<div id="new_related_product_featured" class="row" style='display:none;'>
					<div class="move cell">
						<div class="move_zone"></div>
					</div>
					<div class="image cell">
					<input type=hidden name=related_products_featured[] value=''>
					<img class=product_icon src=''>
					</div>
					<div class="name cell">
					<a class="related_product_name" href=""></a>
			...
			<input type=text name=related id='related_products_featured' class="input_autocomplete" placeholder='Выберите товар чтобы добавить его'>
		</div>

		<div class="block layer" id="exclude_options" style="display:none;">
			<h2>Взаимоисключающие опции</h2>
			<div id=list class="sortable related_products_exclude">
				{foreach from=$excluded_products item=related_product}
				{* Если связанный товар является доп. опцией, то выводим его*}
				{if $related_product->dop || $related_product->featured}
				<div class="row">
					<div class="move cell">
						<div class="move_zone"></div>
					</div>
					<div class="image cell">
					<input type=hidden name=related_products_exclude[] value='{$related_product->id}'>
					<a href="{url id=$related_product->id}">
					<img class=product_icon src='{$related_product->images[0]->filename|resize:35:35}'>
					</a>
					</div>
				...
				{/if}
				{/foreach}
				<div id="new_related_product_exclude" class="row" style='display:none;'>
					<div class="move cell">
						<div class="move_zone"></div>
					</div>
					<div class="image cell">
					<input type=hidden name=related_products_exclude[] value='123'>
					<img class=product_icon src=''>
					</div>
					<div class="name cell">
					<a class="related_product_name" href=""></a>
			...
			<input type=text name=related id='related_products_exclude' class="input_autocomplete" placeholder='Выберите товар чтобы добавить его'>
		</div>
	...
	<!-- Описагние товара -->
	<div class="block layer">
		<h2>Комплект поставки | Монтажные схемы | Сертификаты и РУ</h2>
