...
$(function() {
	...
	$("input#accessories_category").autocomplete({
		serviceUrl:'ajax/search_categories.php',
		minChars:0,
		noCache: false, 
		onSelect:
			function(suggestion){
				$("input#accessories_category_id").val(suggestion.data.id); 
			},
		formatResult:
...
<input type=hidden name="accessories_category_id" id="accessories_category_id" value="{$settings->accessories_category}">
		...
		<div class="block">
			...
			<ul>
				...
				<li><label class=property>Категория вспомогательного оборудования</label><input id="accessories_category" name="accessories_category" class="simpla_inp" type="text" value="{$accessories_category->name|escape}" /></li>
			</ul>
		</div>
