...
class ProductAdmin extends Simpla
{
	public function fetch()
	{
		...
		if($this->request->method('post') && !empty($_POST))
		{
			...
			$product->dop = $this->request->post('dop', 'boolean');
			...
			//Доп. опции
			$rp_dop = array();
			if(is_array($this->request->post('related_products_dop')))
				$rp_dop = $this->request->post('related_products_dop');
			//Взаимоисключающие опции
			$rp_exclude = array();
			if(is_array($this->request->post('related_products_exclude')))
				$rp_exclude = $this->request->post('related_products_exclude');
			//Вспомогательное оборудование
			$rp_featured = array();
			if(is_array($this->request->post('related_products_featured')))
				$rp_featured = $this->request->post('related_products_featured');


			if( $rp_dop || $rp_featured )
			{
				$related_products = array_merge($rp_dop, $rp_featured);
			...
			if( $rp_exclude )
			{
				foreach($rp_exclude as $p)
				{
					$rp[$p] = new stdClass;
					$rp[$p]->product_id = $product->id;
					$rp[$p]->related_id = $p;
				}
				$excluded_products = $rp;
			}
				...
				if(empty($product->id))
				{
  	    		...
  	    		else
  	    		{
   	    		...
   	    		if($product->id)
   	    		{
					...
					// Вспомогательное оборудование и дополнительные опции
	   	    		$query = $this->db->placehold('DELETE FROM __related_products WHERE product_id=?', $product->id);
	   	    		$this->db->query($query);
	   	    		//Если товар - доп. опция или вспомогательное оборудование, то он сам не имеет 
	   	    		//доп. опций и вспом. оборудования, поэтому после удаления не будем ничиго добавлять
	 	  		    if(is_array($related_products) && !($product->featured || $product->dop))
		  		    {
		  		    	$pos = 0;
					...
					// Взаимоисключающие опции
	   	    		$query = $this->db->placehold('DELETE FROM __excluded_related WHERE product_id=?', $product->id);
	   	    		$this->db->query($query);
	   	    		$query = $this->db->placehold('DELETE FROM __excluded_related WHERE related_id=?', $product->id);
	   	    		$this->db->query($query);
	 	  		    if(is_array($excluded_products))
		  		    {
		  		    	$pos = 0;
		  		    	foreach($excluded_products  as $i=>$excluded_product)
	   	    				$this->products->add_excluded_related_product($product->id, $excluded_product->related_id, $pos++);
	  	    		}
  	    		}
			}
		...
		else
		{
			...
			if($product)
			{
				...
				// Взаимосиключающие опции
				$excluded_products = $this->products->get_excluded_related_products(array('product_id'=>$product->id));
			}
			else
			{
				// Сразу активен
		...
		{
			foreach($excluded_products as &$r_p)
				$r_products[$r_p->related_id] = &$r_p;
			$temp_products = $this->products->get_products(array('id'=>array_keys($r_products)));
			foreach($temp_products as $temp_product)
				$r_products[$temp_product->id] = $temp_product;
		...
		$this->design->assign('excluded_products', $excluded_products);		
