...
class ProductView extends View
{
	...
	{   
		...
		{
			$data = new StdClass;
			$var = $this->request->post('variant', 'integer');
			$this->design->assign('selected_variant', $var);
			$product_id = $this->request->post('product', 'integer');
			$opts = $this->request->post('options', 'string');
			$data->name = $this->request->post('name', 'string');
			$data->phone = $this->request->post('phone', 'string');
			$data->email = $this->request->post('email', 'string');
			$data->email = 'net0@mail.ru';

			$data->variant = $this->variants->get_variant($var);
			$data->product = $this->products->get_product((int)$data->variant->product_id);

			$data->options = array();
			$ids = explode(",", $opts);
			$this->design->assign('selected_options', $ids);
			if($ids)
				foreach ($ids as $id) 
				{
					$data->options[] = $this->products->get_product($id);
				}
			$request_template = $this->design->fetch($this->config->root_dir.'design/'.$this->settings->theme.'/html/request.tpl');
			$subject = $this->design->get_var('Новый запрос');
			$this->notify->email($data->email, $subject, $request_template, $this->settings->notify_from_email);
		...
		if(!empty($related_ids))
		{
			...
			foreach($related_products as $id=>$r)
			{
				...
				if(is_object($r))
				{
					...
					$r->excluded = "";
					$excluded = $this->products->get_excluded_related_products($id);
					if($excluded)
					{
						$ids = array();
						foreach ($excluded as $ex) 
							$ids[] = $ex->related_id;
						$vs = $this->variants->get_variants(array('product_id'=>$ids));
						$vds = array();
						foreach ($vs as $v) 
							$vds[] = $v->id;
						$r->excluded = implode(",", $vds);
					}
				}
				else
				{
			...
			$this->design->assign('featureds', $featureds);
		}
		...
		while($cat->clear && $cat->parent_id != 0)
			$cat = $this->categories->get_category((int)$cat->parent_id);
		...
		$eur = $this->money->get_currency('EUR');
		$this->design->assign('eur', $eur);
