...
class ProductsView extends View
{
	...
	function fetch()
	{
		...
		$filter['featured'] = 0;
		$filter['dop'] = 0;
		if($this->settings->accessories_category === $category->id)
			$filter['featured'] = 1;
		...
		if(!empty($category))
		{
			...
			$rps = $this->categories->get_pin_products($category->id);
			$pin_products = array();
			foreach ($rps as $rp) 
			{
				$pin_products[$rp->position] = $this->products->get_product((int)$rp->related_id);
				$images = $this->products->get_images(array('product_id'=>$rp->related_id));
				if($images)
					$pin_products[$rp->position]->image = $images[0];
			}
			
			//$category->subcategories = array_merge($category->subcategories, $pin_products);
			$products = array_merge($products, $pin_products);
		...
		$this->design->assign('eur', $this->money->get_currency('EUR'));
